---
title: 'Equity Analysis of COVID-19 Vaccination in the City of Chicago '
...
Chicago_COVID19Vaccinations_ByZCTA_GEOM_All %>% 
  st_drop_geometry() %>%
  group_by(pcp) %>%
  mutate(total=sum(doses_daily), rate=sum(doses_daily)/Total) %>%
  drop_na(Total) %>%
